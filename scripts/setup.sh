...
if [ "$OPENSHELL_AVAILABLE" = true ]; then
    ...
    echo -e "  3. Start sandboxed:   ${CYAN}hermesclaw mybot start --policy strict${RESET}"
    echo -e "  4. Chat:              ${CYAN}hermesclaw mybot chat \"hello\"${RESET}"
else
    echo -e "${YELLOW}OpenShell not found — required for sandboxed operation.${RESET}"
    echo ""
    echo "Next steps:"
    echo -e "  1. Install OpenShell: ${CYAN}curl -fsSL https://www.nvidia.com/openshell.sh | bash${RESET}"
    echo -e "  2. Download a model:  ${CYAN}curl -L -o models/<model>.gguf <url>${RESET}"
    echo -e "  3. Start llama-server: ${CYAN}llama-server -m models/<model>.gguf --port 8080 --ctx-size 32768 -ngl 99${RESET}"
    echo -e "  4. Start sandboxed:   ${CYAN}hermesclaw mybot start --policy strict${RESET}"
fi
echo ""
