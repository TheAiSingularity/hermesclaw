...
#
# Usage:
#   bash benchmarks/uc-03.sh
#   bash benchmarks/uc-03.sh --ha-url http://homeassistant.local:8123
...
if curl -sf --max-time 5 "$HA_URL/api/" -H "Authorization: Bearer $HA_TOKEN" &>/dev/null 2>&1; then
    ...
    info "Example: bash benchmarks/uc-03.sh --ha-url http://192.168.1.100:8123"
fi
...
if [[ "$HA_AVAILABLE" == "false" ]]; then
    ...
    info "  4. Re-run: bash benchmarks/uc-03.sh"
    echo ""
    info "Next: bash benchmarks/uc-04.sh   # data analyst"
    exit 0
fi
