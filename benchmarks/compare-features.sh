...
#
# Usage:
#   ./benchmarks/compare-features.sh
#   ./benchmarks/compare-features.sh --quick   (skip live inference tests)
...
if ! $QUICK && _container_running; then
    MEMORY_COUNT=$(docker exec hermesclaw find /opt/data/memories -type f 2>/dev/null | wc -l | tr -d ' ')
...
if ! $QUICK && _container_running; then
    SKILL_COUNT=$(docker exec hermesclaw find /opt/data/skills -type d -mindepth 1 -maxdepth 1 2>/dev/null | wc -l | tr -d ' ')
...
*Generated: $TIMESTAMP*
*Run \`./benchmarks/compare-features.sh\` to refresh.*
