...
#
# Run after: ./benchmarks/compare-setup.sh
#
# Usage:
#   bash benchmarks/uc-07.sh
...
MEMORY_CONTENT=$(docker exec hermesclaw cat /opt/data/memories/MEMORY.md 2>/dev/null || echo "")
...
]' | docker exec -i hermesclaw python3 /opt/data/skills/market-alerts/scripts/monitor.py 2>/dev/null || echo "")
...
info "  UC01 researcher:      bash benchmarks/uc-01.sh"
info "  UC02 developer:       bash benchmarks/uc-02.sh"
info "  UC03 home automation: bash benchmarks/uc-03.sh  (needs HA)"
info "  UC04 data analyst:    bash benchmarks/uc-04.sh"
info "  UC05 small business:  bash benchmarks/uc-05.sh"
info "  UC06 privacy:         bash benchmarks/uc-06.sh"
info "  UC07 trader:          bash benchmarks/uc-07.sh  ← you are here"
echo ""
