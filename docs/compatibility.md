...
| **NVIDIA OpenShell** | `0.0.34` (reported working by [@ppritcha](https://github.com/ppritcha) on DGX Spark with HermesClaw v0.3.2) | Not installed by HermesClaw — user installs on host. Version checked by `hermesclaw doctor`. | Policy YAMLs parse against the v1 schema; `openshell sandbox connect` is used by `hermesclaw connect` and does NOT accept `-- COMMAND` (see [#1](https://github.com/TheAiSingularity/hermesclaw/issues/1)). |
...
| v0.3.2 | macOS 14 (Apple Silicon) | v2026.4.23 (via Docker pin) | N/A (Docker fallback) | Maintainer | `hermesclaw chat` + `doctor` pass; kernel-enforcement N/A |

If you test HermesClaw on a fresh platform/version combo, please open a PR adding a row here — or run `hermesclaw doctor` and paste the output into a GitHub issue with the label `field-report`.
...
- **Hermes Agent**: target monthly. Between releases, we hold the pinned tag. Bump process:
  ...
  3. Run `bash benchmarks/compare-features.sh --quick` against the bumped image.
...
1. `hermesclaw doctor` output (includes detected versions)
